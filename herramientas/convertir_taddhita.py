...
# -*- coding: utf-8 -*-
"""
Convierte el maestro `docs/5. Taddhita-Kappa.md` al formato del generador
(`kaccayana/05-taddhita-kappa.md`). Modelo: `herramientas/convertir_samasa.py`
(sesión 59), del que no se aparta en nada: las mismas tres transformaciones,
la misma verificación.

  1. Desglose del encabezado: `[A + B = N voces]` → `[A + B, N]` (los
     escapes de exportación `\+ \= \]` se admiten y se conservan).
  2. TRES bloques por sutta: se inserta un `---` antes del bloque de
     ejemplos, con el mismo retroceso de rótulo de entrada que el Samāsa.
     Los suttas sin ejemplos en lista (§391, §394, §403, §404 y otros)
     quedan en dos bloques, como los del Nāma que no los llevan.
  3. Ejemplos con asterisco → lista numerada `1.`, `2.`, …

El maestro del Taddhita trae las 52 notas ya numeradas `[^n]` en orden de
aparición (tres de ellas dentro de títulos, §354, §356 y §381, como ya
ocurría en el Nāma); el script sólo comprueba que anclas y definiciones
casen. Las fórmulas de cierre del capítulo viajan con el último sutta y no
se tocan.
...
SRC = "docs/5. Taddhita-Kappa.md"
DST = "kaccayana/05-taddhita-kappa.md"
