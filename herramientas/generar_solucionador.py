...
VERSION = "2.4"
FECHA = "2026-09-14"
NOTA_EN = ("THE PASSAGE NOW TRAVELS WITH THE VERDICT. The exported .md did not "
           "say which text each verdict came from, and the page had it in front "
           "of it all along — the passage is in the box when the verdict is "
           "made. Recovering it afterwards is expensive: three batches in a row "
           "had to be traced by hand against the corpus, and two were never "
           "fully recovered, because the repository holds lists of forms and not "
           "the running text of the commentaries. It is stored PER VERDICT and "
           "not «whatever is in the box now», because the board deliberately "
           "outlives a change of passage: one export covered four of them, and "
           "stamping today's text on the older entries would be wrong with an "
           "air of authority, which is worse than silence. Each card keeps its "
           "own, just as it already keeps the signal and the first reading of "
           "the day it was registered. Distinct texts are stored once and the "
           "card points at the number. "
           "Earlier: THIRTY-TWO CASES FROM THE KHAGGAVISĀṆASUTTAVAṆṆANĀ, and one of them "
...
NOTA = ("EL PASAJE VIAJA YA CON EL VEREDICTO. El .md exportado no decía de "
        "qué texto salía cada veredicto, y la página lo tenía delante: el "
        "pasaje está en la caja cuando se adjudica. Reconstruirlo después "
        "cuesta caro —tres lotes seguidos hubo que rastrearlos a mano contra "
        "el corpus, y dos no se recuperaron enteros—, y no siempre se puede, "
        "porque el repositorio tiene listas de formas y no el texto corrido "
        "de los comentarios. Se guarda POR VEREDICTO y no «lo que haya ahora "
        "en la caja», porque el tablero sobrevive al cambio de pasaje a "
        "propósito: un envío abarcó cuatro, y sellar a las fichas antiguas el "
        "texto de hoy sería equivocarse con aplomo, que es peor que callar. "
        "Cada ficha se queda con el suyo, igual que ya se queda con la señal "
        "y con la primera lectura del día en que se registró; los textos "
        "distintos se guardan una vez y la ficha apunta al número. "
        "Antes, en la 2.3: TREINTA Y DOS CASOS DEL KHAGGAVISĀṆASUTTAVAṆṆANĀ, y uno de ellos "
